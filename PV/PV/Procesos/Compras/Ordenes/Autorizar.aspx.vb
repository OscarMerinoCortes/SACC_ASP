...
Public Class Autorizar
	...
	Protected Sub BTNAdministracion_Click(sender As Object, e As ImageClickEventArgs) Handles BTNAdministracion.Click
		MultiView1.SetActiveView(VAdministracion)
	End Sub

	Protected Sub BTNRechazar_Click(sender As Object, e As ImageClickEventArgs) Handles BTNRechazar.Click
		If TBSubtotal.Text <> 0 And TBTotal.Text <> 0 And TBFolio.Text <> "" Then
			cnn.Open()
			sqlQuery = "UPDATE ORDEN_COMPRA SET CONFIRMADA = 'N', COMENTARIO = '" & TBComentarios.Text & "', ENVIARA = '" & TBEnviar.Text & "', ID_USUARIO_AUT = " & 55 & " WHERE ID_ORDEN_COMPRA = " & TBFolio.Text
			Dim objCmd As New SqlCommand(sqlQuery, cnn)
			objCmd.ExecuteNonQuery()
			sqlQuery = "UPDATE COTIZA_REQUI SET ESTADO_ACTUAL = 'X' WHERE NUMOC = " & TBFolio.Text
	...
	Sub Llenar_Lista_Requisiciones()
		cnn.Open()
		sqlQuery = "SELECT OC.Id_Orden_Compra,OC.NUM_ORDEN,OC.Id_Proveedor,P.Nombre,((OC.Total - OC.Discount) + OC.TAX + OC.Freight + OC.Otros_Cargos) AS Total_Pagar,OC.COMENTARIO FROM ORDEN_COMPRA AS OC JOIN PROVEEDOR AS P ON P.Id_Proveedor = OC.Id_Proveedor WHERE OC.Confirmada = 'P' AND OC.Tipo = '" & DDTipoOrdeCompra.SelectedValue & "' ORDER BY NUM_ORDEN"
		Dim objCmd As New SqlCommand(sqlQuery, cnn)
		objCmd.ExecuteNonQuery()
	...
	Protected Sub IMAplicar_Click(sender As Object, e As ImageClickEventArgs) Handles IMAplicar.Click
